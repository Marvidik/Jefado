...
export default function ProductCard({ name, price, originalPrice, discount, rating = 4.5, reviews = 128, badgeColor = '#ef4444', emoji, isNew, isBestSeller }: ProductCardProps) {
    const [wishlisted, setWishlisted] = useState(false);
    const [added, setAdded] = useState(false);
    return (
        <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', overflow: 'hidden', transition: 'all 0.25s', cursor: 'pointer', position: 'relative' }}
        ...
        >
            <div style={{ position: 'relative', background: 'var(--cream)', aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '60px' }}>
                <span style={{ filter: 'drop-shadow(0 4px 8px rgba(56,36,13,0.15))' }}>{emoji}</span>
                <div style={{ position: 'absolute', top: '8px', left: '8px', display: 'flex', flexDirection: 'column', gap: '3px' }}>
                    ...
                    {isBestSeller && <span style={{ background: 'var(--warning)', color: '#fff', fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: '4px' }}>HOT</span>}
                </div>
                <button onClick={() => setWishlisted(!wishlisted)} style={{ position: 'absolute', top: '8px', right: '8px', width: '30px', height: '30px', background: 'var(--surface)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', boxShadow: 'var(--shadow-sm)' }}>
                    {wishlisted ? '❤️' : '🤍'}
                </button>
            </div>
            <div style={{ padding: '11px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '5px' }}>
                    <span style={{ display: 'inline-flex', gap: '1px' }}>{[1, 2, 3, 4, 5].map(i => <span key={i} style={{ fontSize: 12, color: i <= Math.round(rating) ? '#C05800' : 'var(--border)' }}>★</span>)}</span>
                ...
                <button onClick={() => { setAdded(true); setTimeout(() => setAdded(false), 2000); }} style={{ width: '100%', padding: '8px', background: added ? 'var(--success)' : 'var(--primary)', color: '#fff', borderRadius: 'var(--radius)', fontSize: '12px', fontWeight: 700, fontFamily: 'var(--font-body)', transition: 'all 0.2s' }}>
